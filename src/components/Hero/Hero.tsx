...
const Hero = () => {
  ...
  const role = useRoleSwitcher({ roles: ['FULLSTACK DEVELOPER', 'TECH ENTHUSIAST', 'DATA VISIONARY'] })
    ...
    <section className="bg-primary bg-small-glow bg-small-glow-position md:bg-large-glow-position lg:bg-large-glow min-h-[calc(dvh-4rem)] bg-no-repeat">
      <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-4 px-4 pt-12 pb-10 md:grid-cols-2 lg:p-4">
        <div className="flex min-h-48 flex-col justify-between lg:min-h-56 lg:max-w-[33.75rem]">
          <h1>
            <span className="text-neutral mb-2 block text-3xl font-bold">Hi - I'm Aarya Arban</span>
            <span className="text-accent block text-[1.75rem] font-bold">{role}</span>
          </h1>

          <h2 className="text-neutral mt-3">
            Enthusiastic and socially adept individual with a passion for coding, problem-solving, and event management.
          </h2>
...
<a
  href="https://github.com/AaryaArban"
  target="_blank"
...
>
  <svg
    ...
    className="w-4 h-4"
  >
    <path d="M12 0C5.37 0 0 5.373 0 12c0 5.303...Z" />
  </svg>
  Github Profile
</a>
